...
int main(int argc, char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "Error: jpoint <filename>\n");
        exit(-1);
    }
    ...
    if (NULL != main) {
        ...
        JPointInterpreter method(&types);
        void *entry = 0;
        int32_t rc = compileMethodBuilder(&method, &entry);
        if (0 == rc) {
            JPointInterpreterType *jpoint_interpret = (JPointInterpreterType *)entry;
            vm.interpretFunction = (void *)jpoint_interpret;
            ret = jpoint_interpret(&vm, main, nullptr);
        } else {
            fprintf(stderr, "Error generating JPointInterpreter %d\n", rc);
        }
        shutdownJit();
